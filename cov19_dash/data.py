...
def refresh_datasets():
    ...
    time_series_file = data_dir.joinpath("time_series.csv")
    case_data.to_csv(time_series_file, index=False)

    # Save latest day's data
    latest_day_file = data_dir.joinpath("latest_day.csv")
    (
        # Select latest day's data
        case_data.query("Date == @case_data['Date'].max()").to_csv(
            latest_day_file, index=False
        )
    )
...
def load_time_series_data():
    ...
    range of dates.
    """
    file = data_dir.joinpath("time_series.csv")
    if file.exists():
...
def check_if_data_is_stale():
    ...
    if file.exists():
        ...
        data = pd.read_csv(file, nrows=3, parse_dates=["Date"])
